...
namespace MainbotCSharp.Modules
{
    public class MonitorService
    {
        ...
        private class MonitorState
        {
            ...
            public Dictionary<string, string> lastSeen { get; set; } = new();
        }
        ...
        private void LoadState()
        {
            try
            {
                if (File.Exists(STATE_FILE))
                    _state = JsonSerializer.Deserialize<MonitorState>(File.ReadAllText(STATE_FILE)) ?? new MonitorState();
            }
            catch
            {
        ...
        private void SaveState()
        {
            try
            {
                File.WriteAllText(STATE_FILE, JsonSerializer.Serialize(_state, new JsonSerializerOptions { WriteIndented = true }));
            }
            catch
            {
                // Silent fail for now
        ...
        public async Task StartAsync(DiscordSocketClient client)
        {
            ...
            _ = Task.Run(async () =>
            {
                while (true)
                {
                    try
                    {
                        await UpdateAllMonitors(client);
                    }
                    catch (Exception ex)
                    {
                        Console.WriteLine("Monitor update error: " + ex);
                    }
        ...
        private async Task EnsureMonitorMessages(DiscordSocketClient client)
        {
            ...
            foreach (var t in _targets)
            {
                ...
                {
                    if (ulong.TryParse(mid, out var msgId))
                    {
                ...
                var embed = new EmbedBuilder()
                    .WithTitle(t.display)
                    .WithDescription("Initializing status monitor...")
                ...
                if (sent != null)
                {
                    _state.messages[t.key] = sent.Id.ToString();
                    SaveState();
                }
            }
        }
        ...
        private async Task UpdateAllMonitors(DiscordSocketClient client)
        {
            ...
            foreach (var t in _targets)
            {
                try
                {
                    ...
                    if (_state.messages.TryGetValue(t.key, out var mid) && ulong.TryParse(mid, out var msgId))
                    {
                ...
                {
                    Console.WriteLine("per-target monitor error: " + ex);
                }
            }
        }
            ...
            try
            {
                ...
                foreach (var m in guild.Users)
                {
                    ...
                    foreach (var h in hints)
                    {
                        ...
                        if (uname.Contains(lh) || dname.Contains(lh))
                        {
                            found = m;
                            break;
                        }
                ...
                if (found != null)
                {
                    ...
                    if (pres != "offline")
                    {
                        ...
                        _state.lastSeen[hints[0]] = lastSeen;
                        SaveState();
                        return (statusLabel, lastSeen);
                    }

                    if (_state.lastSeen.TryGetValue(hints[0], out var ls) &&
                        (DateTimeOffset.UtcNow - DateTimeOffset.Parse(ls)).TotalMinutes < 5)
                    {
                        statusLabel = "CRASHED";
                    }
                }
